...
if __name__ == '__main__': #to guard the program
    #freeze_support() 
    # Training settings
    ...
    parser.add_argument('--reg_type', type=int, default=0, metavar='R', #set to reg_type instead of reg
    ...
    # Loader
    kwargs = {'num_workers': 1, 'pin_memory': True} if use_cuda else {} #num_workers set to 1 to reduce computational load 
                ...
                if decay:
                    ...
                    for param in model.parameters():
                        if param.requires_grad and torch.sum(torch.abs(param))>0:
                            ...
                            #lp norm based regularization  
                            elif args.reg_type==5:    
                                reg += torch.sum(1+torch.abs(param)**2)
                            elif args.reg_type==6:    
                                reg += torch.sum(1+torch.abs(param)**3)
                            elif args.reg_type==7:    
                                reg += torch.sum(torch.log10(1+torch.abs(param)**2))
                            elif args.reg_type==8:    
                                reg += torch.sum(torch.log10(1+torch.abs(param)**3))
                            elif args.reg_type==9:   
                                reg += torch.sum(torch.log10(1+torch.abs(param)**0.5))
                            elif args.reg_type==10:   
                                reg += torch.sum(torch.log10(1+torch.abs(param)**0.3))
                            elif args.reg_type==11:   
                                reg += torch.sum(torch.log10(1+torch.abs(param)**0.2))
                            elif args.reg_type==12:    
                                reg += torch.sqrt(torch.sum(torch.abs(param)**2))/(torch.sum(torch.abs(param)))
                            elif args.reg_type==13:   
                                reg += torch.sum(torch.log(1+torch.abs(param)**0.2))
                            else:
                                reg = 0.0         
