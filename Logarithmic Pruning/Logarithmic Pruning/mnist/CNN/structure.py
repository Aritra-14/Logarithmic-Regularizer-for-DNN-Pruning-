...
if __name__ == '__main__':
                    ...
                    for name, param in model.named_parameters():
                        if param.requires_grad and 'weight' in name:
                            if args.reg_type==2:
                                ...
                                reg += torch.sum(torch.log(1+torch.abs(param)**1.5))
                            elif args.reg_type==4:   
                                reg += torch.sum(torch.log(1+torch.abs(param)**2.5)) 
    ...
    torch.save(model.state_dict(), 'saves/str_'+str(args.decay)+'_'+str(args.reg_type)+'.pth')
